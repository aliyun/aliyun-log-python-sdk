...
void add_log_timenano_part(log_group_builder * bder, uint32_t logTimeNano)
{
    log_tag * logs = &bder->grp->logs;
    // 1 header and 4 time
    if (bder->grp->log_now_buffer - logs->buffer + 5 > logs->max_buffer_len)
    {
        // reset log_now_buffer
        size_t delta = bder->grp->log_now_buffer - logs->buffer;
        _adjust_buffer(logs, delta + 5);
        bder->grp->log_now_buffer = logs->buffer + delta;
    }
    uint8_t * buf = (uint8_t *)bder->grp->log_now_buffer;
    // timenanao_part (4 << 3 | 5);
    *buf++= 0x25;
    buf += fixed32_pack(logTimeNano, buf);
    bder->grp->log_now_buffer = (char *)buf;
}
